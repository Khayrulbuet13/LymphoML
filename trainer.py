...
class BaseTrainer:
    ...
    def __init__(self, config):
        ...
        # Initialize Comet experiment (optional)
        comet_api_key = getattr(config, 'comet_api_key', None)
        if comet_api_key:
            self.experiment = Experiment(
                api_key=comet_api_key,
                project_name=getattr(config, 'comet_project_name', 'lymphoml'),
            ...
            logging.info("Comet.ml logging disabled (no API key provided)")
...
class TeacherTrainer(BaseTrainer):
    ...
    def setup_data(self):
        ...
        # Load datasets - only download on first instantiation
        ...
        original_val_ds = LymphoMNIST(
            root='./dataset', 
            train=False, 
            download=False,  # Don't download again, dataset already exists
...
class StudentTrainer(BaseTrainer):
    ...
    def setup_data(self):
        ...
        # Load datasets for teacher - only download on first instantiation
        ...
        val_test_ds_teacher = FilteredLymphoMNIST(
            LymphoMNIST(root='./dataset', train=False, download=False, transform=teacher_val_transform, num_classes=3), 
            self.config.labels_to_keep
        )
        
        # Load datasets for student - dataset already downloaded
        train_ds_student = FilteredLymphoMNIST(
            LymphoMNIST(root='./dataset', train=True, download=False, transform=student_train_transform, num_classes=3), 
            self.config.labels_to_keep
        )
        val_test_ds_student = FilteredLymphoMNIST(
            LymphoMNIST(root='./dataset', train=False, download=False, transform=student_val_transform, num_classes=3), 
            self.config.labels_to_keep
        )
